...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'});
 ...
 await page.goto(url);await page.waitForFunction(()=>typeof ClockMath!=='undefined');await page.waitForTimeout(500);
 for(const beat of [30,500]){
  await page.evaluate(b=>{clockTween=null;advanceClock(b,performance.now());},beat);await page.waitForTimeout(600);
  await page.locator('#clockTape').screenshot({path:'.review/tape-'+beat+'.png'});
 }
