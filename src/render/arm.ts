...
export function drawArm<T>(ctx: CanvasRenderingContext2D, segments: readonly ArmSegment<T>[], progress: number, style: ArmDrawStyle<T>) {
  ...
  segments.forEach((segment,i)=>{
    ...
    ctx.strokeStyle=colour;ctx.setLineDash(folded?[3,5]:[]);ctx.lineWidth=highlighted?3.6:2;
    ...
    if(f>0){
      ...
      if(!folded&&Math.hypot(end[0]-a[0],end[1]-a[1])>14){
        const angle=Math.atan2(end[1]-a[1],end[0]-a[0]);ctx.setLineDash([]);ctx.beginPath();
        ctx.moveTo(end[0]-6*Math.cos(angle-.45),end[1]-6*Math.sin(angle-.45));ctx.lineTo(...end);
        ctx.lineTo(end[0]-6*Math.cos(angle+.45),end[1]-6*Math.sin(angle+.45));ctx.stroke();
      }
    }
