...
export function mountWorkspace(actions:WorkspaceActions){
 ...
 const sPanel=createPanel('inputPanel','Input s',`<p class="small-note">Every term is weighted by 1/n<sup>s</sup>. The clock shows the terms; s decides how heavily each one counts.</p><label class="field">σ · real part<input id="clockSigma" type="range" min="0.2" max="3" step="0.001" value="2"><input id="clockSigmaExact" type="number" min="0.2" max="3" step="any" value="2"></label><label class="field">τ · height<input id="clockTau" type="range" min="-35" max="35" step="0.001" value="0"><input id="clockTauExact" type="number" min="-1000" max="1000" step="any" value="0"></label><p class="small-note" id="axisNote">On the real axis the weights are ordinary fractions. Move τ off zero and each term also turns, which is where the curve starts to matter.</p><label class="check"><input id="tauDriveOn" type="checkbox" checked> Turn τ with the beat</label><label class="field">τ per beat<input id="tauRate" type="range" min="-4" max="4" step="0.01" value="1"><output id="tauRateOut">+1.00</output></label><p class="small-note" id="driveNote">τ is riding the clock at +1.00 per beat. Play or step, and every term turns while its length holds.</p><button id="openExplorer">Watch it in the Riemann explorer</button>`);
 const sChip=document.createElement('button');sChip.id='sReadout';sChip.className='text-button';sChip.onclick=()=>open(sPanel.id);el('clockTransport').append(sChip);
 // Leaving the real axis on purpose opens the explorer once; after that the
 // choice is yours. The default drive does not spend that door for you.
 for(const [id,key] of [['clockSigma','sigma'],['clockSigmaExact','sigma'],['clockTau','tau'],['clockTauExact','tau']])el(id).addEventListener('input',()=>{actions.setting(key,el<HTMLInputElement>(id).value);
  if(key==='tau'&&!offAxis&&Math.abs(Number(el<HTMLInputElement>(id).value))>1e-9){offAxis=true;if(layout==='clock')setLayout('both');}});
 bind([['tauDriveOn','tauDrive'],['tauRate','tauRate']]);
 el('tauRate').addEventListener('input',()=>{const v=+el<HTMLInputElement>('tauRate').value;el('tauRateOut').textContent=(v>0?'+':'')+v.toFixed(2);});
 ...
 let lastMode='',lastTime=-1,lastSound:boolean|undefined,lastS='',offAxis=false;
 ...
 const text='s = '+state.sigma.toFixed(2)+(state.tau<0?' − ':' + ')+Math.abs(state.tau).toFixed(2)+'i';
 if(text!==lastS){lastS=text;sChip.textContent=text;for(const [id,value] of [['clockSigma',state.sigma],['clockSigmaExact',state.sigma],['clockTau',state.tau],['clockTauExact',state.tau]] as [string,number][])if(document.activeElement!==el(id))el<HTMLInputElement>(id).value=String(+value.toFixed(6));
