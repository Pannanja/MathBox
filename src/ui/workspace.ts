...
export function mountWorkspace(actions:WorkspaceActions){
 ...
 app.append(shell);el('clockSurface').append(clockStage,el('originalProof'));el('clockTransport').append(el('transportCorner'));el('clockWorkspace').insertBefore(el('fac'),el('clockTransport'));{const rhs=el('originalProof').querySelector('.rhs')!,zeta=rhs.querySelector('.zeta-symbol')!;rhs.prepend(zeta);
 ...
 for(const id of ['viewBubble','tuneBubble','observeBubble','bridgeBubble'])register(el(id),'clock');
 ...
 bind([['allPaneLabels','labels'],['layerFront','layerFront']]);
