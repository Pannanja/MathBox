...
const paths:Record<string,string>={
 ...
 gear:'M12 8.9a3.1 3.1 0 1 0 0 6.2 3.1 3.1 0 0 0 0-6.2ZM20.80 10.13L20.80 13.87L17.92 14.15L17.71 14.66L19.55 16.90L16.90 19.55L14.66 17.71L14.15 17.92L13.87 20.80L10.13 20.80L9.85 17.92L9.34 17.71L7.10 19.55L4.45 16.90L6.29 14.66L6.08 14.15L3.20 13.87L3.20 10.13L6.08 9.85L6.29 9.34L4.45 7.10L7.10 4.45L9.34 6.29L9.85 6.08L10.13 3.20L13.87 3.20L14.15 6.08L14.66 6.29L16.90 4.45L19.55 7.10L17.71 9.34L17.92 9.85Z',
 ...
 // A prism splitting one ray: the spectrum and the light it paints with.
 prism:'M12 4 20.5 18.5h-17ZM1.5 12h5m11.5 1.6 4.5-2.4m-4.5 4.4h4.8m-4.8 2.2 4.5 2.4',
...
export function mountWorkspace(actions:WorkspaceActions){
 ...
 <header class="workspace-title"><h1>Clock of Primes</h1><div class="workspace-actions"><span id="soundSlot"></span><button id="levelsButton" class="icon-button" title="Sound" aria-label="Sound levels">${icon('levels')}</button><button id="lightButton" class="icon-button" title="Light & spectrum" aria-label="Light and spectrum">${icon('prism')}</button><button id="appearanceButton" class="icon-button" title="Clock appearance" aria-label="Clock appearance">${icon('layers')}</button></div></header>
 ...
 app.append(shell);el('clockSurface').append(clockStage,el('originalProof'));el('clockTransport').append(el('transportCorner'));el('clockWorkspace').insertBefore(el('tapeDomain'),el('clockTransport'));el('clockWorkspace').insertBefore(el('fac'),el('clockTransport'));{const rhs=el('originalProof').querySelector('.rhs')!,zeta=rhs.querySelector('.zeta-symbol')!;rhs.prepend(zeta);
 const pi=document.createElement('span');pi.className='tape-op';pi.id='productOp';pi.innerHTML='<span class="factor-ink"><b>∏</b><small>p ≤ <i id="opProductN">1</i></small></span>';zeta.after(pi);
 const sigma=document.createElement('span');sigma.className='tape-op';sigma.id='sumOp';sigma.innerHTML='<span class="factor-ink"><b>∑</b><small>m ≥ <i id="opSumN">1</i></small></span>';el('tape').append(sigma);}
 ...
 el('levelsButton').onclick=()=>open('soundPanel');el('lightButton').onclick=()=>open('lightPanel');el('appearanceButton').onclick=()=>open('viewBubble');el('riemannHelp').onclick=()=>el('explainView').click();
 el('viewBubble').querySelector('.bubble-head b')!.textContent='Clock appearance';el('spec').hidden=true;
 const custom=document.createElement('div');custom.innerHTML=`<label class="field">Depth<input id="layerFront" type="range" min="-1" max="1" step="0.01" value="0"><output id="layerFrontOut">Level</output></label><p class="small-note">The panes are the gearing and the sum is what it drives. Pull either one to the front and the other falls behind a veil, so the dial can be opened up and read through.</p><label class="field">Sum ruler · one unit<input id="sumZoom" type="range" min="0.1" max="2" step="0.01" value="1"><output id="sumZoomOut">σ × 1.00 = 0.50 radii</output></label><p class="small-note">One unit is σ radii: at σ = 1 it reaches the rim, at σ = ½ half way in, so the real part of s is a length you can read against the dial. The factor above is yours on top of that. Neither changes the terms, which stay n<sup>−σ</sup> long.</p><label class="field">ψ departure<input id="stairScale" type="range" min="0" max="1" step="0.01" value="1"><output id="stairScaleOut">magnified</output></label><p class="small-note">The trend rides the clock’s own radius, so the stair climbs out of the origin and the arm writing it stands at the rim. This sets how the departure from that trend is measured. At the right it is carried as (ψ − smooth)/√x, where a riser and a zero’s epicycle are the same size. At the left it is drawn true, on the same radius as the trend — and the whole zero chain shrinks to a few pixels, because ψ is of order x and its correction only of order √x.</p><label class="check"><input id="allPaneLabels" type="checkbox"> All pane labels</label><label class="check"><input id="jRadiusToggle" type="checkbox"> J radius · weighted prime count</label><button id="inspectJ">Inspect J</button>`;el('viewBubble').append(custom);
 ...
 bind([['allPaneLabels','labels'],['jRadiusToggle','jRadius'],['sumZoom','sumZoom'],['layerFront','layerFront'],['stairScale','stairScale']]);
 el('stairScale').addEventListener('input',()=>{const v=+el<HTMLInputElement>('stairScale').value;
  el('stairScaleOut').textContent=v<.02?'true':v>.98?'magnified':'blend '+v.toFixed(2);});
 el('layerFront').addEventListener('input',()=>{const v=+el<HTMLInputElement>('layerFront').value;
  el('layerFrontOut').textContent=v<-.02?'Panes in front':v>.02?'Sum in front':'Level';});
 let rulerSigma=.5;
 const showRuler=()=>{const v=+el<HTMLInputElement>('sumZoom').value;el('sumZoomOut').textContent='σ × '+v.toFixed(2)+' = '+(rulerSigma*v).toFixed(2)+' radii';};
 el('sumZoom').addEventListener('input',showRuler);showRuler();
 ...
 const sPanel=createPanel('inputPanel','Input s',`<p class="small-note">Every term is weighted by 1/n<sup>s</sup>. The clock shows the terms; s decides how heavily each one counts.</p><label class="field">σ · real part<input id="clockSigma" type="range" min="0.2" max="3" step="0.001" value="0.5"><input id="clockSigmaExact" type="number" min="0.2" max="3" step="any" value="0.5"></label><label class="field">τ · height<input id="clockTau" type="range" min="-35" max="35" step="0.001" value="0"><input id="clockTauExact" type="number" min="-1000" max="1000" step="any" value="0"></label><p class="small-note" id="axisNote">On the real axis the weights are ordinary fractions. Move τ off zero and each term also turns, which is where the curve starts to matter.</p><label class="check"><input id="tauDriveOn" type="checkbox" checked> Turn τ with the beat</label><label class="field">τ per beat<input id="tauRate" type="range" min="-4" max="4" step="0.01" value="-1"><input id="tauRateExact" type="number" min="-100" max="100" step="any" value="-1"></label><p class="small-note" id="driveNote">τ is riding the clock at −1.00 per beat. Play or step, and every term turns while its length holds.</p><button id="openExplorer">Watch it in the Riemann explorer</button>`);
 const sChip=document.createElement('button');sChip.id='sReadout';sChip.title='Input s';sChip.setAttribute('aria-expanded','false');
 sChip.innerHTML='<span class="s-name">s</span><span class="s-value">= 0.50 + 0.00i</span>';
 const sValue=sChip.querySelector<HTMLElement>('.s-value')!;
 sChip.onclick=()=>open(sPanel.id);el('clockTransport').append(sChip);
 for(const [id,key] of [['clockSigma','sigma'],['clockSigmaExact','sigma'],['clockTau','tau'],['clockTauExact','tau']])el(id).addEventListener('input',()=>actions.setting(key,el<HTMLInputElement>(id).value));
 bind([['tauDriveOn','tauDrive'],['tauRate','tauRate'],['tauRateExact','tauRate']]);
 // Slider and field are two ways into one rate; each follows the other.
 for(const [from,to] of [['tauRate','tauRateExact'],['tauRateExact','tauRate']])
  el(from).addEventListener('input',()=>{const v=el<HTMLInputElement>(from).value;if(document.activeElement!==el(to))el<HTMLInputElement>(to).value=v;});
 el('openExplorer').onclick=()=>setLayout('both');
 ...
 const soundPanel=createPanel('soundPanel','Sound',`<div id="volumeControl"></div><label class="field">Primes<input id="primeVolume" type="range" min="0" max="100" step="1" value="100"></label><label class="field">Beat<input id="beatVolume" type="range" min="0" max="100" step="1" value="16"></label><label class="field">Beat pitch · Hz<input id="beatPitch" type="range" min="90" max="900" step="5" value="320"></label><label class="field">Prime pitch · Hz per number<input id="pitchMultiplier" type="range" min="1" max="20" step="0.1" value="1"><output id="pitchReadout">1n Hz</output></label><p class="small-note">At 1n Hz the earliest primes are below hearing and arrive as the count grows. Ratios stay proportional, and fast winding skips the backlog of notes.</p>`);
 const lightPanel=createPanel('lightPanel','Light & spectrum',`<label class="field">Hue from<select id="spectrumChoice"><option value="pitch">Pitch · log₂ p</option><option value="wave">Wavelength · 1/pᵏ</option></select></label><div id="spectrumPreview" aria-label="Prime colour preview"></div>
 <label class="field">Exponent k<input id="spectrumExp" type="range" min="0.3" max="3" step="0.05" value="1"><output id="spectrumExpOut">1.00</output></label>
 <label class="field">Spread<input id="spectrumSpread" type="range" min="0.2" max="4" step="0.05" value="1"><output id="spectrumSpreadOut">1.00</output></label>
 <label class="field">Rotate<input id="spectrumRotate" type="range" min="0" max="359" step="1" value="0"><output id="spectrumRotateOut">0°</output></label>
 <p class="small-note">Pitch repeats the wheel once per octave, so near primes contrast strongly and distant ones collide. Wavelength runs once from red to violet, so contrast concentrates in the small primes. Spread stretches either mapping; rotate turns the whole wheel.</p>
 <label class="field">Pane light<select id="lightChoice"><option value="off">Off · bare sieve</option><option value="filter" selected>Filtering glass</option><option value="light">Accumulating light</option></select></label><label class="field">Pane brightness<input id="colourDepth" type="range" min="0.1" max="1.2" step="0.01" value="0.42"></label><label class="check"><input id="weightBrightness" type="checkbox"> Link pane brightness to σ</label><p class="small-note">Colour is a display mapping. ξ does not control brightness, and no finite palette separates every factorization.</p>`);void lightPanel;
 ...
 bind([['primeVolume','primeVolume'],['beatVolume','beatVolume'],['beatPitch','beatPitch'],['colourDepth','depth'],['weightBrightness','weightBrightness'],['spectrumChoice','spectrum'],['lightChoice','light'],['spectrumExp','spectrumExp'],['spectrumSpread','spectrumSpread'],['spectrumRotate','spectrumRotate']]);
 for(const [id,suffix] of [['spectrumExp',''],['spectrumSpread',''],['spectrumRotate','°']])el(id).addEventListener('input',()=>{el(id+'Out').textContent=(suffix?Math.round(+el<HTMLInputElement>(id).value):(+el<HTMLInputElement>(id).value).toFixed(2))+suffix;});void soundPanel;
 ...
 const sigmaRow=el('sigmaDial').closest('.axis-control') as HTMLElement,tauRow=el('tauScrub').closest('.axis-control') as HTMLElement;sigmaRow.id='sigmaControl';tauRow.id='tauControl';graphWrap.append(sigmaRow,tauRow);
 sigmaRow.title='Input s · real part';tauRow.title='Input s · height';
 el('paperViewTitle').hidden=true;el('paperEquation').hidden=true;el('planeHint').hidden=true;
 // Name the three quantities the explorer keeps apart: the input, the interval
 // sampled for the curve, and the camera.
 const section=(title:string,node:HTMLElement)=>{const h=document.createElement('h3');h.className='option-heading';h.textContent=title;node.before(h);node.hidden=false;};
 el('exploreOptions').append(el('traceWindow'),el('productRange').closest('.product-tools') as HTMLElement);
 section('ζ curve · τ sampled',el('traceWindow'));
 section('View range · output camera',el('productRange').closest('.product-tools') as HTMLElement);
 ...
 let lastMode='',lastTime=-1,lastSound:boolean|undefined,lastS='';
 ...
 // τ runs as far as the evaluator does, so its slider follows the value rather
 // than fencing it in: a fine control around where you are, not a ceiling.
 const tauSlider=el<HTMLInputElement>('clockTau');
 if(document.activeElement!==tauSlider){const low=Math.max(-1000,Math.min(960,state.tau-20));tauSlider.min=String(low);tauSlider.max=String(low+40);}
 if(Math.abs(rulerSigma-state.sigma)>.004){rulerSigma=state.sigma;showRuler();}
 sChip.setAttribute('aria-expanded',String(el('inputPanel').dataset.open==='true'));
 const text='= '+state.sigma.toFixed(2)+(state.tau<0?' − ':' + ')+Math.abs(state.tau).toFixed(2)+'i';
 if(text!==lastS){lastS=text;sValue.textContent=text;for(const [id,value] of [['clockSigma',state.sigma],['clockSigmaExact',state.sigma],['clockTau',state.tau],['clockTauExact',state.tau]] as [string,number][])if(document.activeElement!==el(id))el<HTMLInputElement>(id).value=String(+value.toFixed(6));
  el('axisNote').textContent=Math.abs(state.tau)<1e-9?'On the real axis the weights are ordinary fractions. Move τ off zero and each term also turns, which is where the curve starts to matter.':'Off the real axis every term turns as well as shrinks. The explorer draws where those turns land.';}
