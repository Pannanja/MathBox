...
export function mountWorkspace(actions:WorkspaceActions){
 ...
 const custom=document.createElement('div');custom.innerHTML=`<label class="check"><input id="allPaneLabels" type="checkbox"> All pane labels</label><label class="check"><input id="jRadiusToggle" type="checkbox"> J radius · weighted prime count</label><button id="inspectJ">Inspect J</button>`;el('viewBubble').append(custom);
 ...
 bind([['allPaneLabels','labels'],['jRadiusToggle','jRadius']]);
 ...
 // Leaving the real axis opens the explorer once; after that the choice is yours.
 if(!offAxis&&Math.abs(state.tau)>1e-9){offAxis=true;if(layout==='clock')setLayout('both');}
 },open};
}
