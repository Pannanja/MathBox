...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'});try{
 ...
 for(const mode of ['euler','log','xiarm','term','finite','infinite','theta']){
  await pickView(page,mode);await page.waitForTimeout(150);
 ...
 await pickView(page,'log');await page.waitForTimeout(100);await page.screenshot({path:'.review/explorer-orientation.png'});
 ...
 for(const [width,height] of [[390,844],[568,320],[320,568]]){
  ...
  // The catalogue is collapsed until asked for; open it to check its entries.
  await page.evaluate(()=>{document.getElementById('constructionChooser').open=true;});await page.waitForTimeout(100);
  ...
  await page.evaluate(()=>{document.getElementById('constructionChooser').open=false;});
  if(width>height){assert.ok(await page.evaluate(()=>{const a=$('clockTransport').getBoundingClientRect(),b=$('exploreToolbar').getBoundingClientRect();return a.bottom<=b.top||a.top>=b.bottom||a.right<=b.left||a.left>=b.right;}));}
  await page.screenshot({path:'.review/explorer-orientation-'+width+'.png'});
 }
 await page.locator('#exploreMore').click();await page.locator('#openPlaneNotes').click();assert.ok(await page.locator('#planeNotes').evaluate(e=>e.querySelector('.bubble-head').getBoundingClientRect().top<e.querySelector('.coordinate-note').getBoundingClientRect().top));assert.deepEqual(errors,[]);console.log('All seven views: shared axes/strip, visible names, unchanged clock/input/ruler, full vectors, offscreen directions and responsive navigation passed.');
